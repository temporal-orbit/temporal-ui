...
export const ProgressLinear = forwardRef<HTMLDivElement, ProgressLinearProps & ProgressRootProps>(
	(props, ref) => {
		const {
			label,
			hint,
				...
				<ProgressRoot {...rootProps} ref={ref} data-testid={testId ? `${testId}--root` : undefined}>
					{children ?? (
						<>
							...
							{showValueText && (
								<ProgressValueText data-testid={testId ? `${testId}--value-text` : undefined} />
							)}
						</>
					)}
				</ProgressRoot>
